...
Hooks.once("ready", () => {
    ...
    game.settings.register("wfrp4e-print-sheet", "conection", {
		...
		type: String,
		choices: {
			1: game.i18n.localize("WFRP4E.PrintSheet.Settings.conection.Host"),
			2: game.i18n.localize("WFRP4E.PrintSheet.Settings.conection.Local"),
			3: game.i18n.localize("WFRP4E.PrintSheet.Settings.conection.Internet")
		}
	});
		...
		if (dataActor) {
			...
			data.status = {
				...
				criticals: (await Promise.all(dataActor._itemTypes.critical.map(async (c) => ({
					name: c.name,
					image: game.settings.get("wfrp4e-print-sheet", "embedimages") ? await DataMapper.getImage(c.img) : false,
					...
					name: i.name,
					image: game.settings.get("wfrp4e-print-sheet", "embedimages") ? await DataMapper.getImage(i.img) : false,
					...
					name: d.name,
					image: game.settings.get("wfrp4e-print-sheet", "embedimages") ? await DataMapper.getImage(d.img) : false,
				...
				effects: (await Promise.all(dataActor.effects.filter(e => !e.flags.assistant).map(async (e) => ({
					...
					image: game.settings.get("wfrp4e-print-sheet", "embedimages") ? await DataMapper.getImage(e.img) : false,
			...
			data.talents = (await Promise.all(dataActor._itemTypes.talent.map(async (t) => ({
				name: t.name,
				image: game.settings.get("wfrp4e-print-sheet", "embedimages") ? await DataMapper.getImage(t.img) : false,
			...
			data.psychologys = await Promise.all(dataActor._itemTypes.psychology.map(async (p) => ({
				name: p.name,
				image: game.settings.get("wfrp4e-print-sheet", "embedimages") ? await DataMapper.getImage(p.img) : false,
			...
			data.mutations = await Promise.all(dataActor._itemTypes.mutation.map(async (m) => ({
				name: m.name,
				image: game.settings.get("wfrp4e-print-sheet", "embedimages") ? await DataMapper.getImage(m.img) : false,
			...
			data.armours = await Promise.all(dataActor._itemTypes.armour.concat(dataActor._itemTypes["wfrp4e-archives3.armour"]).filter(a => !a.location.value).map(async (a) => ({
				...
				image: game.settings.get("wfrp4e-print-sheet", "embedimages") ? await DataMapper.getImage(a.img) : false,
			...
			data.weapons = (await Promise.all(dataActor._itemTypes.weapon.filter(w => !w.location.value).map(async (w) => ({
				...
				image: game.settings.get("wfrp4e-print-sheet", "embedimages") ? await DataMapper.getImage(w.img) : false,
				group: w.WeaponGroup,
...
<div style='display: flex; justify-content: center;'>
	<table style='border: 1px solid var(--color-large_border); border-collapse: collapse; font-size: 12px;'>
		...
		<tbody>
			...
			data.magics = (await Promise.all(dataActor._itemTypes.spell.map(async (s) => ({
				name: s.name,
				image: game.settings.get("wfrp4e-print-sheet", "embedimages") ? await DataMapper.getImage(s.img) : false,
				cn: s.cn.value,
	...
	static async getImage(url) {
		if (url != "systems/wfrp4e/icons/blank.png") {
			url = (game.settings.get("wfrp4e-print-sheet", "conection") == 1 ? ("http://localhost:" + game.data.options.port + "/") : (game.settings.get("wfrp4e-print-sheet", "conection") == 2 ? game.data.addresses.local : game.data.addresses.remote)) + url;
		...
		for (let i = 0; i < containers.length; i++) {
			if (!containers[i].location.value || containers[i].location.value == location) {
				resultData.push({
					...
					image: game.settings.get("wfrp4e-print-sheet", "embedimages") ? await DataMapper.getImage(containers[i].img) : false,
		...
		for (let i = 0; i < items.length; i++) {
			if (items[i].type != "money" && (!items[i].location.value || items[i].location.value == location)) {
				let item = {
					...
					image: game.settings.get("wfrp4e-print-sheet", "embedimages") ? await DataMapper.getImage(items[i].img) : false,
