export default class DataMapper { 
	...
	static mapSheetDataToDataExport(dataSheet = {}){
		let dataExport = {};
		
		dataExport.settings = {editable: "", inputcolor: ""}
		...
		switch (game.settings.get("wfrp4e-print-sheet", "inputcolor")) {
			...
			case 3:
				...
				break;
			case 4:
				dataExport.settings.inputcolor = game.settings.get("wfrp4e-print-sheet", "inputcolorcustom")
				break;
		}
		
		dataExport.name = dataSheet.name;
		dataExport.race = "";
		switch (dataSheet.system.details.species.value) {
			case "human":
				dataExport.race = game.i18n.localize("Human");
				break;
			case "dwarf":
				dataExport.race = game.i18n.localize("Dwarf");
				break;
			case "halfling":
				dataExport.race = game.i18n.localize("Halfling");
				break;
			case "helf":
				dataExport.race = game.i18n.localize("High Elf");
				break;
			case "welf":
				dataExport.race = game.i18n.localize("Wood Elf");
				break;
			default:
				dataExport.race = dataSheet.system.details.species.value;
		}
		...
		dataExport.experience = {current: dataSheet.system.details.experience.current, spent: dataSheet.system.details.experience.spent, total: dataSheet.system.details.experience.total};
		...
		dataExport.skills = DataMapper.mapWFRP4eSkillsDataToDataExport(dataSheet.itemTypes.skill, dataSheet.system.characteristics);
		dataExport.talents = DataMapper.mapWFRP4eTalentsDataToDataExport(dataSheet.itemTypes);
		...
		dataExport.armours = DataMapper.mapWFRP4eArmoursDataToDataExport(dataSheet.itemTypes.armour);
		dataExport.avatar = dataSheet.img.slice(dataSheet.img.lastIndexOf("/") + 1);
		
		dataExport.trappings = DataMapper.mapWFRP4eTrappingsDataToDataExport(dataSheet.itemTypes);
		
		dataExport.psychologys = []
		for (let i = 0; i < dataSheet.itemTypes.psychology.length ; i++) {
			dataExport.psychologys.push({name: dataSheet.itemTypes.psychology[i].name});
		}
		dataExport.mutations = []
		for (let i = 0; i < dataSheet.itemTypes.mutation.length ; i++) {
			dataExport.mutations.push({name: dataSheet.itemTypes.mutation[i].name});
		}
		...
		dataExport.notes = dataSheet.system.details.biography.value.replace(/<[^>]*>/g, '');
		
		dataExport.weapons = DataMapper.mapWFRP4eWeaponsDataToDataExport(dataSheet.itemTypes);
		
		dataExport.magics = DataMapper.mapWFRP4eMagicsDataToDataExport(dataSheet.itemTypes, dataSheet.system.status.sin.value);
		
		return dataExport;
	}
	
	static mapWFRP4eSkillsDataToDataExport(dataWFRP4eSkills = {}, dataWFRP4eCharacteristics) {
	...
	static mapWFRP4eTalentsDataToDataExport(dataWFRP4e = {}) {
	...
	static mapWFRP4eArmoursDataToDataExport(dataWFRP4eArmours = {}) {
	...
	static mapWFRP4eTrappingsDataToDataExport(dataWFRP4e = {}) {
	...
	static mapWFRP4eWeaponsDataToDataExport(dataWFRP4e = {}) {
	...
	static mapWFRP4eMagicsDataToDataExport(dataWFRP4e = {}, sin) {
